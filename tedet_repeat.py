...
def logInsertion(filename, result):
    ...
    for r in results:
        align1, align2 = r[0:2]
        data = align1.refChr, align1.myName, align1.myEnd, align2.myStart
        print(*data, sep='\t', end='\n', file=outfile)


def findTE(args):
    repeats = readRepeats(args[0])
    insertions = readAlignments(args[1])
    ...
    outputfile = args[2]
    #logInsertion(outputfile + "_insertions", insertions)
    #logResult(outputfile + "_unmapped", unmapped, 'unmapped')
    TEpositve, TEnegative= findAnnotated(insertions, repeats)
    logResult(outputfile + "_TEmapped", TEpositve, 'TEmapped')
    logResult(outputfile + "_TEunmapped", TEnegative, 'TEunmapped')

    statistics = Alignment.aligncount, len(TEpositve), len(TEnegative)
    logResult(outputfile + "_statistics", statistics, 'statistics')
